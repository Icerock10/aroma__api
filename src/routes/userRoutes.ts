...
userRouter.route(UserRoutes.GOOGLE_AUTH).get(userController.googleAuth);
userRouter.route(UserRoutes.GOOGLE_AUTH_CALLBACK).get(userController.googleAuthCallback);
...
userRouter
  .route(UserRoutes.UPLOAD_AVATAR)
  .post(multerUploadToStorage.single(SINGLE_IMAGE), userController.uploadAvatar);
